...
class Program
{
    ...
    static void Main()
    {
        List<Debtor> debtors = new List<Debtor> {
            ...
            new Debtor("Martin S. Malinowski", DateTime.Parse("January 17, 1992"), "7765-599-3523", "MartinSMalinowski@dayrep.com", "3749 Capitol Avenue New Castle, IN 47362", 1816),
            ...
            new Debtor("Pamela H. Beauchamp", DateTime.Parse("November 20, 1959"), "801-559-63747", "PamelaHBeauchamp@jourrapide.com", "3239 Tori Lane Salt Lake City, UT 84104", 6588)
        };
        //2
        ...
        //3
        //var db = debtors.Where(d => d.BirthDay.Year > 1988 && d.BirthDay.Year < 1998).ToList();
        //db.ForEach(debtors => Console.WriteLine(debtors));

        //4
        //var db=debtors.Where(d=>d.Debt<=5000).ToList();
        //db.ForEach(debtors => Console.WriteLine(debtors));

        //5
        //    var db = debtors.Where(d => d.FullName.Length > 18 && d.Phone.Count(c => c == '7') >= 2)

        //.ToList();
        //    db.ForEach(d => Console.WriteLine(d));

        //7
        //var db = debtors.Where(d => d.BirthDay.Month == 12 || d.BirthDay.Month == 1 || d.BirthDay.Month == 2).ToList();
        //db.ForEach(debtors => Console.WriteLine(debtors));

        //8
        //double average = debtors.Average(d => d.Debt);
        //var db=debtors.Where(d=>d.Debt>average)
        ...
        //var db = debtors.Where(d => !d.Phone.Contains("8"))
        //      .Select(d => new { FullName = d.FullName, Age = 2024 - d.BirthDay.Year, Debt = d.Debt })
        //    .ToList();
        //db.ForEach(debtors => Console.WriteLine(debtors));

        //11
        //var db=debtors.Where(d=>d.FullName.GroupBy(c=>c).Any(g=>g.Count()>=3))
        //    .OrderBy(d=>d.FullName).ToList();
        //db.ForEach(debtors=>Console.WriteLine(debtors));

        //13
        //var dbMostYear = debtors.GroupBy(d => d.BirthDay.Year)
        //    .OrderByDescending(g => g.Count())
        //    .First()
        //    .Key;
        //Console.WriteLine($"en cox {dbMostYear} ilinde dogum olmusdur");

        //14
        //var top5=debtors.OrderByDescending(d=>d.Debt).Take(5).ToList();
        //top5.ForEach(debtors=>Console.WriteLine(debtors));

        //15
        //var totalSum = debtors.Sum(d => d.Debt);
        //Console.WriteLine($"Total sum of debtors {totalSum}");

        //16
        //var db=debtors.Where(d=>d.BirthDay.Year<1945).ToList();
        //db.ForEach(debtors=>Console.WriteLine(debtors));

        //18
        //var db=debtors.Where(d=>d.Phone.Distinct().Count()==d.Phone.Length)
        //    .Select(d=>new { FullName = d.FullName, Debt = d.Debt })
        //    .ToList();
        //db.ForEach(debtors=>Console.WriteLine(debtors));
        //19
        var db=debtors.Where(d=>d.Debt<=6000).ToList();
        db.ForEach(debtors=>Console.WriteLine(debtors));

        //20
        //string word = "smile";
        //var db=debtors.Where(d=>d.FullName.ToLower().Contains(word)).ToList();
        //db.ForEach(debtors=>Console.WriteLine(debtors));
